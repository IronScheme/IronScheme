...
appveyor AddTest "Release bootstrap" -Outcome Running
isc ironscheme-buildscript.sps
@IF %ERRORLEVEL% NEQ 0 exit /b 1
isc ironscheme-buildscript.sps
peverify /nologo ironscheme.boot.dll
@IF %ERRORLEVEL% NEQ 0 exit /b 1
appveyor UpdateTest "Release bootstrap" -Outcome Passed

appveyor AddTest "Release libraries" -Outcome Running
isc compile-system-libraries.sps
ren lib lib.hide
isc --show-loaded-libraries compile-system-libraries.sps > libraries.lst
ren lib.hide lib
appveyor UpdateTest "Release libraries" -Outcome Passed

appveyor AddTest "Debug bootstrap" -Outcome Running
isc -debug ironscheme-buildscript.sps
@IF %ERRORLEVEL% NEQ 0 exit /b 1
isc -debug ironscheme-buildscript.sps
peverify /nologo /ignore=0x80131820 ironscheme.boot.dll
@IF %ERRORLEVEL% NEQ 0 exit /b 1
appveyor UpdateTest "Debug bootstrap" -Outcome Passed

appveyor AddTest "Debug libraries" -Outcome Running
...
ren lib.hide lib
appveyor UpdateTest "Debug libraries" -Outcome Passed

appveyor AddTest "Release bootstrap for test" -Outcome Running
isc ironscheme-buildscript.sps
FOR /F %%f IN ('type libraries.lst') DO @del %%f
appveyor UpdateTest "Release bootstrap for test" -Outcome Passed

appveyor AddTest "Release libraries for test" -Outcome Running
isc compile-system-libraries.sps
appveyor UpdateTest "Release libraries for test" -Outcome Passed

appveyor AddTest "R6RS" -Outcome Running
isc tests\r6rs\run.sps
isc tests\trigtest.sps
isc tests\clisp-number-tests.sps
isc tests\fp-test.sps
appveyor UpdateTest "R6RS" -Outcome Passed

appveyor AddTest "SRFI" -Outcome Running
...
appveyor UpdateTest "SRFI" -Outcome Passed

appveyor AddTest "PFDS" -Outcome Running
isc lib\pfds\tests.scm
appveyor UpdateTest "PFDS" -Outcome Passed

appveyor AddTest "MiniKanren" -Outcome Running
echo (include "lib/minikanren/mktests.scm") | isc
appveyor UpdateTest "MiniKanren" -Outcome Passed
