...
namespace gppg {
 ...
 where TSpan : IMerge<TSpan>, new() {
        ...
        {
          return string.Format("unexpected {0} in '{1}' at {2}", TerminalToString(NextToken), filename, GetLocation(LastSpan));
        }
        ...
        private void ReportError() {
            ...
            if (FsaState.ParserTable.Count < 20) {
                bool first = true;
          ...
          {
            scanner.Errors.Add(scanner.SourceUnit, errorMsg.ToString(), GetLocation(LastSpan), 1, Microsoft.Scripting.Hosting.Severity.Error);
          }
          System.Diagnostics.Trace.WriteLine(errorMsg.ToString());
        }

        protected abstract Microsoft.Scripting.SourceSpan GetLocation(TSpan lastL);
...
#endif
 {
        ...
        public LexLocation Merge(LexLocation last)
        {
          ...
          var s = Math.Min(startLine * (uint.MaxValue + 1L) + startColumn, last.startLine * (uint.MaxValue + 1L) + last.startColumn);
          var e = Math.Max(endLine * (uint.MaxValue + 1L) + endColumn, last.endLine * (uint.MaxValue + 1L) + last.endColumn);
